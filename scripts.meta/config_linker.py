...
def _run_wsl_command_helper(command_parts: list, dry_run: bool = False) -> bool:
    ...
    try:
        ...
        result = subprocess.run(command_parts, capture_output=True, text=True, check=False, encoding='utf-8', errors='replace')
        if result.stdout.strip():
            print(f"    STDOUT: {result.stdout.strip()}")
        if result.stderr.strip():
            print(f"    STDERR: {result.stderr.strip()}") # rm -rf usually silent on success
        ...
        print(f"  Exception while executing WSL command '{' '.join(command_parts)}': {e}")
        return False

def _run_windows_cmd_via_pwsh_helper(cmd_command_parts_list: list, dry_run: bool = False) -> bool:
    """
    ...
    full_pwsh_command = ['pwsh.exe', '-NoProfile', '-Command', f'cmd /c {cmd_command_str_for_pwsh}']

    print(f"  INTERNAL WIN CMD (via pwsh): {' '.join(full_pwsh_command)}")
    if dry_run:
        print("    (DRY_RUN _run_windows_cmd_via_pwsh_helper) Command not actually executed.")
        return True
    ...
    try:
        ...
        print(f"  Exception while executing Windows command '{cmd_command_str_for_pwsh}': {e}")
        return False
...
def create_windows_symlink(
    ...
    try:
        ...
        print(f"  ERROR: WSL item '{wsl_item_path}' is not inside the WSL config root '{wsl_config_root_path}'.")
        print(f"         Cannot determine relative path for C: drive mapping.")
        return False
    ...
    try:
        windows_item_via_mnt_c.parent.mkdir(parents=True, exist_ok=True) # Ensure parent directory exists
    ...
    try:
        item_exists_on_windows_side = path_to_delete_obj.exists() or path_to_delete_obj.is_symlink()
    ...
    if item_exists_on_windows_side:
        if dry_run:
            print(f"    (DRY_RUN) Would target '{windows_item_via_mnt_c_str}' for deletion.")
        else: # Not dry_run and item exists, so consider actual deletion
            if confirm_deletion and path_to_delete_obj.is_dir(): # Prompt for any directory (real or symlink to dir)
                user_confirm = input(f"    CONFIRM DELETION of directory '{windows_item_via_mnt_c_str}' on /mnt/c? (yes/no): ").lower()
                if user_confirm != 'yes':
                    print(f"    User skipped deletion of '{windows_item_via_mnt_c_str}'. Link creation will be skipped for this item.")
            ...
            if not _run_wsl_command_helper(['rm', '-rf', windows_item_via_mnt_c_str], dry_run=False): # Actual deletion attempt
                print(f"  ERROR: Deletion of '{windows_item_via_mnt_c_str}' failed.")
                return False # Hard failure
    else: # Item does not exist on /mnt/c
        print(f"  Original item '{windows_item_via_mnt_c_str}' not found on /mnt/c. No deletion needed.")
    ...
    wsl_item_abs_path_for_unc = str(wsl_item_path).replace('/', '\\') # wsl_item_path is already absolute & resolved
...
def main_cli():
    ...
    parser.add_argument(
        "--ci", "--non-interactive", # Added --ci alias
        action="store_true",
        dest="ci_mode",
    ...
    try:
        resolved_wsl_config_root = str(Path(args.wsl_config_root).expanduser().resolve(strict=True))
    ...
    for p_str in args.wsl_paths:
        try:
            ...
            print(f"Error: Input WSL path '{p_str}' does not exist. Skipping this path.")
    ...
    if args.ci_mode:
        effective_confirm_deletion = False # CI mode implies no interactive confirmation for deletions
        print("Running in CI mode (non-interactive, no confirmations for individual directory deletions).")
    ...
    if not args.dry_run and not args.ci_mode:
        ...
        print(f"This script will attempt to process {len(resolved_wsl_paths)} WSL item(s).")
        print(f"For each item, it may delete content on your Windows C: drive (via /mnt/c/)")
        print(f"and then create a symbolic link from Windows to WSL.")
        print(f"  WSL paths to process:")
        for rp in resolved_wsl_paths: print(f"    - {rp}")
        ...
        if effective_confirm_deletion:
            print(f"  Confirmation will be asked before deleting existing directories on /mnt/c.")
        else:
            print(f"  Confirmation for deleting existing directories on /mnt/c is OFF.")
        print("---")
        try:
            if effective_confirm_deletion:
                proceed_all = input("Do you want to proceed with the overall operation? (yes/no): ").lower()
                if proceed_all != 'yes':
    ...
    print(f"Successfully processed (or skipped by user choice without error): {success_count} item(s)")
