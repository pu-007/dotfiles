...
_update:
    ...
    command -v pkgfile &>/dev/null && sudo pkgfile --update 2>/dev/null || true
    command -v nvim &>/dev/null && nvim --headless "+Lazy! sync" +qa || true
    command -v npm &>/dev/null && sudo npm install -g @google/gemini-cli aicommit2 || true
...
_sync_remote:
    ...
    # 使用 --autostash，自动将 _update 生成的改动 (如 lazy-lock.json) 闪电隐藏并在 pull 完后恢复，解决冲突报错
    git pull --rebase --autostash || echo -e "{{c_yellow}}⚠️ Pull 遇到问题，尝试继续 Push...{{c_reset}}"
